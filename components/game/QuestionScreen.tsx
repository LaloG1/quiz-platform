...
interface Answer { id: string; text: string; is_correct: boolean; }
interface Question { id: string; question: string; answers: Answer[]; time_limit: number; }
...
const COLORS = [
  { bg: 'bg-[#E21B3C]', border: 'border-[#b31530]', hover: 'hover:bg-[#c01530]', shape: '▲' }, // Rojo
  { bg: 'bg-[#1368CE]', border: 'border-[#0f52a3]', hover: 'hover:bg-[#0f52a3]', shape: '◆' }, // Azul
  { bg: 'bg-[#D89E00]', border: 'border-[#b38300]', hover: 'hover:bg-[#b38300]', shape: '●' }, // Amarillo
  { bg: 'bg-[#26890C]', border: 'border-[#1e6b0a]', hover: 'hover:bg-[#1e6b0a]', shape: '■' }, // Verde
];
...
export default function QuestionScreen({ question, isHost, onPlayerAnswer }: QuestionScreenProps) {
  ...
  }, [isHost, answered, question.time_limit]);
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-5xl mx-auto p-4 h-full">
      ...
      <div className="w-full bg-white rounded-2xl p-6 mb-6 shadow-xl text-center relative overflow-hidden">
        {!isHost && (
          <div className="absolute top-0 left-0 h-2 bg-[#46178F] transition-all duration-1000" 
               style={{ width: `${(timeLeft / question.time_limit) * 100}%` }} />
        )}
        <h2 className="text-2xl md:text-4xl font-black text-[#46178F] mt-2">{question.question}</h2>
        {!isHost && <p className="text-xl font-bold text-gray-500 mt-2">{timeLeft}s</p>}
      </div>
